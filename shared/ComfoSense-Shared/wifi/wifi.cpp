...
void WiFi::configure(const WiFiConfig& cfg) {
    this->config = cfg;
...
void WiFi::setup() {
    if (!configured) {
        Serial.println("WiFi: ERROR - not configured! Call configure() before setup()");
        return;
    }
...
void WiFi::loop() {
    ...
    if (now - last_status_check >= STATUS_CHECK_INTERVAL) {
        ...
        // Only handle state changes not caught by events
...
void WiFi::updateWiFiIcon() {
    if (wifi_icon_callback) {
        wifi_icon_callback(connected);
    }
}
