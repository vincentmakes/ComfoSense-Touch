...
```
> [!IMPORTANT]
> *Note from vincentmakes*
> 
> I've had some troubles with the original automations.yaml: with this more complex bridge, this was creating an echo or feedback which was driving the fan speed of the ComfoAir
> all over the place. This updated yaml fixes the issue with one caveat is that only actual user input can send MQTT commands. This means no automation or one would need to find
> another solution.
